...
# planning_csv = str(snakemake.input.planning_summary)
# dispatch_csv = str(snakemake.input.dispatch_summary)
# output_dir = str(snakemake.output.compare_output)
...
#         df.index = df[grouping[0]]
#         if len(grouping) > 1:
#             for g in grouping[1:]:
#                 df.index = df.index + "|" + df[g]
#     else:
...
#     _pre = lambda *a, **k: pre_process_results(
#         compare_table_planning, scenarios, *a, **k
#     )
...
#     all_results.to_csv(os.path.join(output_dir, "results.csv"), index=False)
